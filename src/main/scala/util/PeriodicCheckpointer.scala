 ...
 */
abstract class PeriodicCheckpointer[T](val checkpointInterval: Int, val sc: SparkContext) {
...
object PeriodicCheckpointer {
  ...
  def removeCheckpointFile(checkpointFile: String, conf: Configuration): Unit = {
    try {
      ...
      case e: Exception =>
        println("PeriodicCheckpointer could not remove old checkpoint file: " +
          checkpointFile)
    }
  }
}
