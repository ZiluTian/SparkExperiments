...
object SimulateMessageRDD extends util.SparkTest {
  def apply(actors: List[Actor], totalTurn: Int): Unit = {
    ...
    while (currentTurn < totalTurn ) {
        ...
        actorRDD = actorRDD.leftOuterJoin(messageRDD).mapValues(x => {
          x._1.run(x._2.getOrElse(List()))
        }).cache()
        
        messageRDD.unpersist()
        messageRDD = actorRDD.flatMap(x => x._2.sendMessages.map(m => (m._1, m._2))).reduceByKey((m1, m2) => m1 ::: m2)
